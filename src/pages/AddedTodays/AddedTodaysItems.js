...
const AddedTodaysItems = ({ items }) => {
    ...
    return (
        <div style={{  backgroundImage:`linear-gradient(180deg, rgba(255,255,255,0) 50%, rgba(0,0,0,0.6997308623924684) 100%), url('${items?.img1}')`   }} className='todayItem-container d-flex flex-column justify-content-between'>
            ...
            <div className='d-flex justify-content-between align-items-end'>
                ...
                <div className='todaysAdded-btn'>
                    <button className='bg-transparent text-light fw-bold border-0'>Price $ {items?.price} </button> <i className="far fa-heart"></i>
                </div>
            </div>
        </div>
    );
